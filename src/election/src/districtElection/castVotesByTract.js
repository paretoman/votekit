...
export default function castVotesByTract(geometry0, electionOptions) {
    ...
    const { canPoints, parties, dimensions, geography, strategySeed, voterGroupBehaviorList, information } = geometry0
    ...
    const votesByTract = voterGeomsByTract.map(
        (row) => row.map(
            (voterGeoms) => {
                const geometry = { voterGeoms, canPoints, parties, dimensions, canBorders, strategySeed, voterGroupBehaviorList, information }

import { cos, evaluate, matrix, min, multiply, pi, sin, sqrt, transpose, zeros } from "mathjs"
...
let rs = [.1,.1,.1] // radius
// let rs = [1,1,1] // radius
let dt = [1,1,1] // distribution type
...
function beatMap(c,v) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        const mp = quantile(vp.toArray()[0],rs,dt,.25)
        ...
        // const b = add(c, ba)
        let b = evaluate(`c + 2 * d * (mp - d' * c)`,{c,d,mp,d,c})
